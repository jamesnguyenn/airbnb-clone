...
import { currentDomain } from '../axios/config';
...
export async function getStaticProps() {
    const exploreData = await fetch(`${currentDomain}/api/explore`).then(
        (res) => res.json()
    );
    const liveAnyWhereData = await fetch(
        `${currentDomain}/api/liveanywhere`
    ).then((res) => res.json());
    return {
        props: {
            exploreData: exploreData.data,
            liveAnyWhereData: liveAnyWhereData.data,
        },
    };
}
